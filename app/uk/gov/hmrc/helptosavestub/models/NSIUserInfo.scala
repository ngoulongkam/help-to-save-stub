...
object NSIUserInfo {
  ...
  def apply(createAccount: CreateAccount): ValidatedNel[String, NSIUserInfo] =
    (forenameValidation(createAccount.forename) |@|
      ...
      countryCodeValidation(createAccount.countryCode) |@|
      ninoValidation(createAccount.NINO) |@|
      emailValidation(createAccount.emailAddress.getOrElse(""))).map(
  ...
  private def addressLineValidation(address: Address): ValidatedNel[String, ValidatedAddressLines] = {
    val list = List(address.line1, address.line2, address.line3,
      address.postcode, address.country).collect{ case Some(s) ⇒ s}.filter(_.nonEmpty)
  ...
  private def ninoValidation(nino: NINO): ValidatedNel[String, NINO] = {
    val lengthCheck =
